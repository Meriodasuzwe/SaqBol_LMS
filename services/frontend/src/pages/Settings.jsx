...
function Settings() {
    ...
    const fetchProfile = async () => {
        try {
            ...
            toast.error(t('settings.errFetchProfile'));
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleProfileSubmit = async (e) => {
        ...
        try {
            ...
            if (profileData.email !== originalEmail) {
                ...
                toast.info(t('settings.successOtpSent'));
            } else {
                toast.success(t('settings.successProfileUpdate'));
            }
        } catch (error) {
            const errorMsg = error.response?.data?.error || t('settings.errSaveProfile');
            toast.error(errorMsg);
            if (error.response?.data?.error?.includes('используется') || error.response?.data?.error?.includes('used')) {
    ...
    const handleVerifyOtp = async (e) => {
        e.preventDefault();
        if (otpCode.length !== 6) {
            toast.warning(t('settings.errOtpLength'));
            return;
        }
        setVerifying(true);
        try {
            await api.post('users/verify-email-change/', { code: otpCode });
            toast.success(t('settings.successEmailUpdate'));
            setOriginalEmail(profileData.email); 
            setShowOtpModal(false);
            setOtpCode('');
        } catch (error) {
            toast.error(error.response?.data?.error || t('settings.errInvalidOtp'));
        } finally {
            setVerifying(false);
        }
    };
    ...
    return (
        <div className="max-w-5xl mx-auto py-10 px-4 sm:px-6 lg:px-8 font-sans text-base-content transition-colors duration-200" style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
            ...
            <div className="mb-8">
                <h1 className="text-3xl font-extrabold tracking-tight">{t('settings.pageTitle')}</h1>
                <p className="text-base-content/60 font-medium mt-2">{t('settings.pageSubtitle')}</p>
            </div>
                ...
                <div className="w-full md:w-64 shrink-0">
                    <nav className="flex md:flex-col gap-2 overflow-x-auto pb-4 md:pb-0">
                        <button onClick={() => setActiveTab('profile')} className={`flex items-center gap-3 px-4 py-3 rounded-xl font-bold transition-all whitespace-nowrap ${activeTab === 'profile' ? 'bg-blue-100 dark:bg-blue-900/40 text-blue-800 dark:text-blue-400 border border-blue-200 dark:border-blue-800/50' : 'text-base-content/70 hover:bg-base-200 hover:text-base-content border border-transparent'}`}>
                            <User size={20} /> {t('settings.tabProfile')}
                        </button>
                        <button onClick={() => setActiveTab('security')} className={`flex items-center gap-3 px-4 py-3 rounded-xl font-bold transition-all whitespace-nowrap ${activeTab === 'security' ? 'bg-blue-100 dark:bg-blue-900/40 text-blue-800 dark:text-blue-400 border border-blue-200 dark:border-blue-800/50' : 'text-base-content/70 hover:bg-base-200 hover:text-base-content border border-transparent'}`}>
                            <Shield size={20} /> {t('settings.tabSecurity')}
                        </button>
                    </nav>
                </div>
                        ...
                        <div className="bg-base-100 rounded-3xl border border-base-300 shadow-sm overflow-hidden transition-colors duration-200">
                            ...
                            {activeTab === 'profile' && (
                                <div className="p-8 animate-in fade-in duration-300">
                                    <h2 className="text-xl font-extrabold mb-6">{t('settings.profileTitle')}</h2>
                                    {isFakeEmail && (
                                        <div className="mb-8 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800/50 rounded-2xl p-5 flex gap-4 items-start">
                                            ...
                                            <div>
                                                <h3 className="text-amber-900 dark:text-amber-400 font-bold text-sm">{t('settings.fakeEmailTitle')}</h3>
                                                <p className="text-amber-800 dark:text-amber-200/70 text-sm mt-1">{t('settings.fakeEmailDesc')}</p>
                                            </div>
                                        </div>
                                    )}
                                    ...
                                    <form onSubmit={handleProfileSubmit} className="space-y-6">
                                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                                            <div>
                                                <label className="block text-xs font-bold text-base-content/50 uppercase tracking-wider mb-2">{t('settings.firstName')}</label>
                                            ...
                                            <div>
                                                <label className="block text-xs font-bold text-base-content/50 uppercase tracking-wider mb-2">{t('settings.lastName')}</label>
                                        ...
                                        <div>
                                            <label className="block text-xs font-bold text-base-content/50 uppercase tracking-wider mb-2">{t('settings.email')}</label>
                                        ...
                                        <div>
                                            <label className="block text-xs font-bold text-base-content/50 uppercase tracking-wider mb-2">{t('settings.username')}</label>
                                        ...
                                        <div className="pt-4 border-t border-base-200 flex justify-end">
                                            <button type="submit" disabled={saving} className="flex items-center gap-2 px-6 py-3 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-xl transition-all shadow-lg shadow-blue-200 dark:shadow-blue-900/20">
                                                {saving ? <div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin"></div> : <Save size={18} />} {t('settings.saveBtn')}
                                            </button>
                                        </div>
                                    </form>
                                </div>
                            ...
                            {activeTab === 'security' && (
                                <div className="p-8 animate-in fade-in duration-300">
                                    <h2 className="text-xl font-extrabold mb-6">{t('settings.securityTitle')}</h2>
                                    <form onSubmit={handlePasswordSubmit} className="space-y-6">
                                        ...
                                        <div>
                                            <label className="block text-xs font-bold text-base-content/50 uppercase tracking-wider mb-2">{t('settings.oldPassword')}</label>
                                            <div className="relative">
                                        ...
                                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                                            <div>
                                                <label className="block text-xs font-bold text-base-content/50 uppercase tracking-wider mb-2">{t('settings.newPassword')}</label>
                                                <div className="relative">
                                                    <input type={showNew ? "text" : "password"} required minLength={6} value={passwordData.new_password} onChange={e => setPasswordData({...passwordData, new_password: e.target.value})} className={inputClasses} placeholder={t('settings.minChars')} />
                                                    <button type="button" onClick={() => setShowNew(!showNew)} className={eyeBtnClasses}>{showNew ? <EyeOff size={18} /> : <Eye size={18} />}</button>
                                                </div>
                                            </div>
                                            <div>
                                                <label className="block text-xs font-bold text-base-content/50 uppercase tracking-wider mb-2">{t('settings.confirmPassword')}</label>
                                                <div className="relative">
                                                    <input type={showConfirm ? "text" : "password"} required value={passwordData.confirm_password} onChange={e => setPasswordData({...passwordData, confirm_password: e.target.value})} className={inputClasses} placeholder={t('settings.repeatPassword')} />
                                        ...
                                        <div className="pt-4 border-t border-base-200 flex justify-end">
                                            <button type="submit" disabled={saving} className="flex items-center gap-2 px-6 py-3 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-xl transition-all shadow-lg shadow-blue-200 dark:shadow-blue-900/20 disabled:opacity-70">
                                                {saving && <div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin"></div>} {t('settings.updatePasswordBtn')}
                                            </button>
                                        </div>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in">
                    <div className="bg-base-100 rounded-3xl shadow-2xl w-full max-w-md overflow-hidden">
                        <div className="p-6 border-b border-base-200 flex items-center justify-between">
                            <h3 className="font-extrabold text-lg text-base-content">{t('settings.otpModalTitle')}</h3>
                        ...
                        <form onSubmit={handleVerifyOtp} className="p-6 space-y-6">
                            <p className="text-base-content/80 text-sm">{t('settings.otpModalDesc1')}<strong className="text-base-content">{profileData.email}</strong>{t('settings.otpModalDesc2')}</p>
                            <div>
                                <input 
                                    type="text" 
                                    maxLength={6}
                            ...
                            <button type="submit" disabled={verifying || otpCode.length !== 6} className="w-full flex justify-center items-center py-4 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-xl transition-all disabled:opacity-50">
                                {verifying ? <div className="w-6 h-6 border-2 border-white/30 border-t-white rounded-full animate-spin"></div> : t('settings.verifyCodeBtn')}
                            </button>
                        </form>
                    </div>
                </div>
