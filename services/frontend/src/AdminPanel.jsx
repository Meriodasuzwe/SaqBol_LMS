...
import {
    ...
    AlertCircle, Send, ArrowRight, Eye, Briefcase, Mail, Key, Trash2, Bug, ExternalLink, MessageSquare, Link as LinkIcon
} from 'lucide-react';
...
const AdminPanel = () => {
    ...
    const [allPublishedCourses, setAllPublishedCourses] = useState([]); // 🔥 Для выпадающего списка
    ...
    const loadData = async () => {
        ...
        try {
            if (activeTab === 'apps') {
                ...
                // Загружаем заявки
                const resLeads = await api.get('courses/b2b/leads/');
                setLeads(resLeads.data);
                // Загружаем список всех курсов для выпадающего списка
                const resCourses = await api.get('courses/');
                setAllPublishedCourses(resCourses.data);
    ...
    // 🔥 ФУНКЦИЯ ПРИВЯЗКИ КУРСА К ЗАЯВКЕ
    const handleAssignCourse = async (leadId, courseId) => {
        if (!courseId) return;
        try {
            const res = await api.patch(`courses/b2b/leads/${leadId}/update/`, { target_course: courseId });
            setLeads(leads.map(l => l.id === leadId ? res.data : l));
            toast.success('Курс успешно привязан!');
        } catch (err) {
            toast.error('Ошибка привязки курса');
        }
    };
                                ...
                                <div key={lead.id} style={{ background: themeColors.panel, borderRadius: 20, padding: 20, border: `1px solid ${themeColors.panelBdr}`, display: 'flex', alignItems: 'center', gap: 20 }}>
                                    ...
                                    <div style={{ flex: 1, minWidth: 0 }}>
                                        ...
                                        {lead.course_title ? (
                                            ...
                                            <div style={{ marginBottom: 8, display: 'flex', alignItems: 'center', gap: 8 }}>
                                                <div style={{ fontSize: 11, fontWeight: 800, padding: '4px 10px', borderRadius: 8, background: isDark ? 'rgba(245,158,11,0.1)' : '#fef3c7', color: '#f59e0b', textTransform: 'uppercase' }}>
                                                    Курс не выбран
                                                </div>
                                                <select 
                                                    onChange={(e) => handleAssignCourse(lead.id, e.target.value)}
                                                    defaultValue=""
                                                    style={{
                                                        padding: '2px 8px', borderRadius: '6px', border: `1px solid ${themeColors.divider}`, background: themeColors.bg, color: themeColors.text, fontSize: '11px', outline: 'none', cursor: 'pointer'
                                                    }}
                                                >
                                                    <option value="" disabled>Привязать курс...</option>
                                    ...
                                    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 12, flexShrink: 0 }}>
                                        ...
                                        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                                            ...
                                            {/* Кнопка генерации кода (всегда на месте) */}
                                            {generatedCodes[lead.id] ? (
                                                ...
                                                <button
                                                    ...
                                                    title={!lead.course_title ? "Сначала выберите курс из списка" : lead.status === 'rejected' ? "Заявка отклонена" : "Сгенерировать код"}
                                                    style={{ 
                                                        padding: '8px 12px', 
                                                        borderRadius: 10, 
