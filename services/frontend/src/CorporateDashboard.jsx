...
const CorporateDashboard = () => {
    ...
    const executeRevokeAccess = async () => {
        ...
        try {
            ...
            toast.success("Доступ отозван, 1 место освобождено!");
            setRevokeConfirmData(null); 
        } catch (error) {
            toast.error(error.response?.data?.error || "Ошибка при отзыве доступа");
        } finally {
            setIsRevoking(false);
        }
    };
    ...
    if (data.length === 0) {
        return (
            <div className="min-h-screen bg-base-200 p-8 flex flex-col items-center justify-center text-center">
                ...
                <h2 className="text-2xl font-black mb-2">Кабинет руководителя</h2>
                <p className="text-base-content/60 max-w-md">У вас пока нет активных заявок. Оставьте заявку на обучение, чтобы получить доступ к аналитике.</p>
            ...
            <div className="bg-base-100 border-b border-base-300 sticky top-0 z-10">
                <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h1 className="text-xl font-black">Кабинет руководителя</h1>
                            <p className="text-xs text-base-content/50 uppercase tracking-widest font-bold">Ваши заявки и аналитика</p>
                        </div>
                    </div>
                        ...
                        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-6">
                            <div>
                                ...
                                <p className="text-xs text-base-content/50">
                                    Заявка от {new Date(lead.created_at).toLocaleDateString()}
                                </p>
                            </div>

                            {/* 🔥 ТЕПЕРЬ СНАЧАЛА ПРОВЕРЯЕМ НА ОТКЛОНЕНИЕ 🔥 */}
                            {lead.status === 'rejected' ? (
                                <div className="flex items-center gap-2 px-4 py-2 bg-red-50 dark:bg-red-900/30 text-red-700 dark:text-red-400 rounded-xl border border-red-200 dark:border-red-800">
                                    <XCircle size={18} />
                                    <span className="font-bold text-sm">Отклонена</span>
                                </div>
                            ) : lead.invite_code ? (
                                <div className="flex items-center gap-2 px-4 py-2 bg-emerald-50 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400 rounded-xl border border-emerald-200 dark:border-emerald-800">
                                    ...
                                    <span className="font-bold text-sm">Доступ открыт</span>
                                </div>
                            ) : (
                                <div className="flex items-center gap-2 px-4 py-2 bg-amber-50 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400 rounded-xl border border-amber-200 dark:border-amber-800">
                                    <Clock size={18} />
                                    <span className="font-bold text-sm">Ожидает оформления</span>
                                </div>
                            )}
                        </div>

                        {/* 🔥 ТЕПЕРЬ СНАЧАЛА ПРОВЕРЯЕМ НА ОТКЛОНЕНИЕ ТУТ ТОЖЕ 🔥 */}
                        {lead.status === 'rejected' ? (
                            <div className="pt-6 border-t border-base-200 text-center py-8">
                                <div className="w-16 h-16 bg-red-100 dark:bg-red-900/30 text-red-500 rounded-full flex items-center justify-center mx-auto mb-4">
                                    <XCircle size={32} />
                                </div>
                                <h3 className="font-bold text-lg mb-2 text-base-content">Заявка отклонена</h3>
                                <p className="text-base-content/60 text-sm max-w-md mx-auto">
                                    К сожалению, администратор закрыл доступ к этому корпоративному курсу. Если вы считаете, что произошла ошибка, пожалуйста, свяжитесь с нашей службой поддержки.
                                </p>
                            </div>
                        ) : lead.invite_code ? (
                            <>
                                <div className="flex flex-wrap gap-4 mb-8 pt-6 border-t border-base-200">
                                    <div className="bg-base-200/50 p-4 rounded-2xl border border-base-200 flex-1 min-w-[200px]">
                                        <p className="text-[10px] uppercase font-black text-base-content/50 tracking-widest mb-1">Код доступа</p>
                                    ...
                                    <div className="bg-base-200/50 p-4 rounded-2xl border border-base-200 flex-1 min-w-[150px]">
                                        <p className="text-[10px] uppercase font-black text-base-content/50 tracking-widest mb-1">Использовано мест</p>
                                ...
                                <div className="flex items-center justify-between mb-6">
                                    <h3 className="text-lg font-bold flex items-center gap-2">
                                        <Users size={18} className="text-base-content/50" /> Статистика сотрудников
                                    </h3>
                                ...
                                <div className="overflow-x-auto">
                                    <table className="w-full text-left text-sm">
                                        <thead className="text-[10px] uppercase font-black text-base-content/40 tracking-widest bg-base-200/50">
                                            <tr>
                                                <th className="px-6 py-4 rounded-l-xl">Сотрудник</th>
                                                <th className="px-6 py-4">Прогресс</th>
                                                <th className="px-6 py-4">Статус</th>
                                                <th className="px-6 py-4 rounded-r-xl text-right">Действия</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            {lead.employees
                                            ...
                                            {lead.employees.length === 0 && (
                                                <tr>
                                                    <td colSpan="4" className="px-6 py-12 text-center text-base-content/40 font-medium">
                                                        Пока ни один сотрудник не активировал код доступа
                                                    </td>
                                                </tr>
                                            )}
                                        </tbody>
                            ...
                            <div className="pt-6 border-t border-base-200 text-center py-8">
                                ...
                                <h3 className="font-bold text-lg mb-2">Заявка на рассмотрении</h3>
                                <p className="text-base-content/60 text-sm max-w-md mx-auto">
                                    Наш менеджер скоро свяжется с вами для оформления доступа. Как только код будет сгенерирован, статистика ваших сотрудников появится здесь.
                                </p>
                            </div>
                        )}
                    </div>
            ...
            {revokeConfirmData && (
                <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
                    ...
                    <div className="bg-base-100 rounded-[2rem] p-8 w-full max-w-md relative z-10 shadow-2xl border border-base-300 animate-in zoom-in-95 duration-200">
                        ...
                        <h3 className="text-2xl font-black mb-2 text-center text-base-content">Отозвать доступ?</h3>
                        <p className="text-center text-base-content/70 mb-8 text-sm">
                            Вы собираетесь забрать доступ к курсу у сотрудника <span className="font-bold">{revokeConfirmData.employee.name}</span>. 
                            Его прогресс сохранится в базе, но он больше не сможет войти в материалы курса. 
                            <br/><br/>
                            <span className="inline-block px-3 py-1 bg-emerald-100 text-emerald-700 rounded-md font-bold text-xs">
                                +1 место вернется на ваш баланс
                            </span>
                        </p>
                        ...
                        <div className="flex gap-3">
                            ...
                            >
                                {isRevoking ? <div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin"></div> : "Да, отозвать"}
                            </button>
                        </div>
                    </div>
                </div>
                    ...
                    <div className="bg-base-100 rounded-[2rem] p-8 w-full max-w-2xl relative z-10 shadow-2xl border border-base-300 max-h-[90vh] flex flex-col">
                        ...
                        <div className="grid grid-cols-2 gap-4 mb-8 shrink-0">
                            <div className="bg-base-200/50 p-4 rounded-xl border border-base-200">
                                <p className="text-[10px] uppercase font-black text-base-content/50 tracking-widest mb-1">Текущий прогресс</p>
                                <div className="flex items-center gap-3">
                            ...
                            <div className="bg-base-200/50 p-4 rounded-xl border border-base-200">
                                <p className="text-[10px] uppercase font-black text-base-content/50 tracking-widest mb-1">Статус обучения</p>
                        ...
                        <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar">
                            ...
                            {selectedEmployee.tests && selectedEmployee.tests.length > 0 ? (
                                ...
                                <div className="text-center py-8 bg-base-200/30 rounded-xl border border-dashed border-base-300">
                                    <p className="text-sm text-base-content/50 font-medium">Сотрудник пока не прошел ни одного теста.</p>
                                </div>
                            )}
                        </div>
                    </div>
