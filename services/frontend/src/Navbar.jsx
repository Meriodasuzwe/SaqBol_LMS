...
function Navbar({ isLoggedIn, userRole, onLogout }) {
    ...
    const submitAuthor = async () => {
        ...
        try {
            ...
            toast.success('Заявка отправлена! ' + r.data.message);
    ...
    const NavLink = ({ to, icon, children }) => {
        ...
        return (
            <Link to={to}
                style={{ color: active ? '#3b82f6' : t.textMute, fontFamily: 'inherit' }}
    ...
    const mobileItems = [
        ...
        { to: '/admin',             icon: <ShieldCheck size={15} />,     label: 'Модерация',         show: isAdmin },
    ].filter(i => i.show);
                ...
                <div style={{ maxWidth: 1280, margin: '0 auto', padding: '0 2rem', height: 64, display: 'flex', alignItems: 'center', gap: 24 }}>
                    ...
                    {isLoggedIn && (
                        <nav style={{ display: 'flex', alignItems: 'center', gap: 24, marginLeft: 8 }} className="hidden lg:flex">
                            ...
                            {isTeacher
                                ? <NavLink to="/analytics/teacher" icon={<BarChart2 size={14} style={{ opacity: 0.7 }} />}>Аналитика</NavLink>
                                : <NavLink to="/analytics/student" icon={<BarChart2 size={14} style={{ opacity: 0.7 }} />}>Мой прогресс</NavLink>
                            }
                        </nav>
                    )}
                    ...
                    <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 8 }}>
                        ...
                        {/* КОЛОКОЛЬЧИК */}
                        {isLoggedIn && <NotificationBell isDark={isDark} t={t} />}
                        ...
                        {isLoggedIn ? (
                            <div className="hidden lg:block" style={{ position: 'relative' }} ref={dropRef}>
                                ...
                                {dropOpen && (
                                    <div className="sq-drop" style={{ position: 'absolute', right: 0, top: 'calc(100% + 8px)', width: 260, background: t.panel, border: `1px solid ${t.panelBdr}`, borderRadius: 18, overflow: 'hidden', boxShadow: isDark ? '0 20px 60px rgba(0,0,0,0.6)' : '0 20px 60px rgba(0,0,0,0.12)', zIndex: 50 }}>
                                        ...
                                        <div style={{ padding: '6px' }}>
                                            {dropItems.map(item => (
                                                <Link key={item.to} to={item.to}
                                                    style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 12px', borderRadius: 12, textDecoration: 'none', color: t.text, fontSize: 13, fontWeight: 500, transition: 'background 0.12s', background: isActive(item.to) ? (isDark ? 'rgba(59,130,246,0.1)' : '#eff6ff') : 'transparent' }}
                                                    ...
                                                    <div style={{ width: 28, height: 28, borderRadius: 8, background: t.iconBg, display: 'flex', alignItems: 'center', justifyContent: 'center', color: t.textMute, flexShrink: 0 }}>{item.icon}</div>
                                                    {item.label}
                                                </Link>
                                            ))}
                        ...
                        {isLoggedIn && (
                            <button onClick={() => setMobileOpen(o => !o)} className="lg:hidden"
                                ...
                                {[0,1,2].map(i => (
                                    <span key={i} style={{ display: 'block', width: 16, height: 2, background: t.textMute, borderRadius: 2, transition: 'all 0.2s',
                    ...
                    <div className="sq-drop" style={{ position: 'relative', background: t.panel, border: `1px solid ${t.panelBdr}`, borderRadius: 20, width: '100%', maxWidth: 440, overflow: 'hidden', boxShadow: isDark ? '0 25px 80px rgba(0,0,0,0.7)' : '0 25px 80px rgba(0,0,0,0.15)' }}>
                        <div style={{ padding: '20px 24px 18px', borderBottom: `1px solid ${t.divider}` }}>
                            <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
                                ...
                                <button onClick={() => setModalOpen(false)} style={{ width: 30, height: 30, borderRadius: 8, border: 'none', cursor: 'pointer', background: 'transparent', display: 'flex', alignItems: 'center', justifyContent: 'center', color: t.textMute }}><X size={14} /></button>
                            </div>
                        </div>
                        <div style={{ padding: '20px 24px', display: 'flex', flexDirection: 'column', gap: 16 }}>
                            <div>
                                <label style={{ fontSize: 11, fontWeight: 700, color: t.textMute, textTransform: 'uppercase', letterSpacing: '0.08em', display: 'block', marginBottom: 8 }}>О вас и вашем опыте <span style={{ color: '#ef4444' }}>*</span></label>
                                <textarea rows={4} placeholder="Расскажите о вашем опыте. Какие курсы планируете создать?"
                                    value={cvText} onChange={e => setCvText(e.target.value)}
                                    style={{ width: '100%', padding: '12px 14px', background: t.input, border: `1px solid ${t.inputBdr}`, borderRadius: 12, fontSize: 13, color: t.text, outline: 'none', resize: 'none', boxSizing: 'border-box', fontFamily: 'inherit' }}
                                    onFocus={e => e.target.style.borderColor = '#3b82f6'}
                                    onBlur={e => e.target.style.borderColor = t.inputBdr} />
                            </div>
                            <div>
                                <label style={{ fontSize: 11, fontWeight: 700, color: t.textMute, textTransform: 'uppercase', letterSpacing: '0.08em', display: 'block', marginBottom: 8 }}>Портфолио / LinkedIn</label>
                                <div style={{ position: 'relative' }}>
                                    ...
                                    <input type="url" placeholder="https://linkedin.com/in/ваше-имя"
                                        ...
                                        style={{ width: '100%', padding: '12px 14px 12px 36px', background: t.input, border: `1px solid ${t.inputBdr}`, borderRadius: 12, fontSize: 13, color: t.text, outline: 'none', boxSizing: 'border-box', fontFamily: 'inherit' }}
                                        onFocus={e => e.target.style.borderColor = '#3b82f6'}
                                        onBlur={e => e.target.style.borderColor = t.inputBdr} />
                                </div>
                            </div>
                        ...
                        <div style={{ padding: '0 24px 24px', display: 'flex', gap: 10 }}>
                            <button onClick={() => setModalOpen(false)} style={{ flex: 1, padding: '12px', border: `1px solid ${t.inputBdr}`, borderRadius: 12, fontSize: 13, fontWeight: 700, color: t.text, background: 'transparent', cursor: 'pointer' }}>Отмена</button>
